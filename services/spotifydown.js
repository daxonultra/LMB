...
async function downloadSpotifySong(songId, chatId, replyToMessageId, { bot, Spotify, channelId }) {
    console.log(`\n🎵 Processing Spotify Video ID: ${songId}`);

    // Temp file paths
    const tempAudioPath = path.join(TEMP_DIR, `${songId}_temp.mp3`);
    ...
    try {
        ...
        const videoDetails = await getData(spotifyUrl);

        const title = videoDetails.name || 'Unknown Title';
        const artist = videoDetails.artists?.[0]?.name || videoDetails.artist || 'Unknown Artist';
        const durationMs = videoDetails.duration || 0;
        const duration = Math.floor(durationMs / 1000); // seconds
        const thumbnail =
          videoDetails.visualIdentity?.image?.find(img => img.maxWidth === 640)?.url
          || videoDetails.visualIdentity?.image?.[0]?.url
          || null;
        ...
        console.log('🔗 Getting download link...');
         ...
         const downloadUrl = `https://spotdown.org/api/direct-download?url=https://open.spotify.com/track/${songId}`;
...
module.exports = { downloadSpotifySong };
